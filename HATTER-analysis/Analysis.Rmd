...
df_filtered_max  <- df_byTime %>% 
  group_by(date) %>% 
  summarise(entries=n(), n_windows = max(n_windows), n_tabs=max(n_tabs), .groups = 'drop') 
  
#filter for min num tabs per day
df_filtered_min  <- df_byTime %>% 
  group_by(date) %>% 
  summarise(entries=n(), n_windows = min(n_windows), n_tabs=min(n_tabs), .groups = 'drop')         
df_filtered <- rbind(df_filtered_max,df_filtered_min) %>% 
  arrange(date)  


# df_byUser <- df_byTime %>% 
#         group_by(user) %>% 
#         summarize(samples=n(), total_windows=sum(n_windows), total_tabs=sum(n_tabs))
...
ggplot(df_byTime, aes(x=n_windows, y=n_tabs)) +
...
ggplot(df_filtered, aes(x=n_windows, y=n_tabs)) +
    geom_point(shape=20, alpha=0.2) +
    geom_smooth(method=lm) +  # Add a loess smoothed fit curve with confidence region
    geom_jitter() +
    labs(y="number of tabs", x = "number of windows")+
...
#RESHAPE data 
m_filtered <-gather(df_filtered,'n_windows','n_tabs',key="type",value="count")


numdates <- nrow(df_filtered)

ggplot(m_filtered, aes(x=date, y=count, color=type)) +
    geom_point(shape=20, alpha=1) +
    scale_y_continuous(breaks = pretty_breaks(n = 8)) +
    # scale_x_continuous(breaks = pretty_breaks(n = numdates)) +
    scale_x_date(date_breaks = "5 day", date_labels = "%D",date_minor_breaks = "1 day")+
